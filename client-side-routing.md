...
对于后端渲染，路由是由后端处理的。当 URL 路径改变或者请求参数改变的时候会输出一个新的页面，这对于传统网页是完美的解决方案。然而网页程序经常需要保持当前用户的状态，这在服务端渲染的页面之间是很难维护的。
...
他们只是在 url 改变的时候运行适当的路由处理程序，从来启动和渲染所需的组件。前端和服务端的结构是类似的，唯一的区别即是处理函数所做的事情。
...
NX 控件组件--以及许多其他组件--可以通过声明性配置连接到参数池，它会决定参数如何和组件的状态，URL，浏览器历史和网页存储进行交互。
